...
def testScenarioDict_parser(inputFile,logging):
    ...
    for index,row in wb.iterrows():
        ...
        iterate_TS=0
        while( row.index[iterate_TS] != 'TestStep#1'):
            iterate_TS+=1
...
#
    ...
    f.close()
#    print("**********************************************")
#    for key,value in testScenarioDict.items():
#        print(" Key : {0} - Value : {1}".format(key,value)) 
...
def help():
    ...
    print("\t Arg 4 : Time in mins")
    print("\t Reference : python <parser>.py <NGAImportSheet>.xlsx <n> <order> <time>")
...
def arguments():
    global inputFile,groups,order,time_Mins
    if (len(sys.argv) < 5):
        help()
...
#
    ...
    # RUN Above commands on HOST
    #cmdlist = []
    #cmdlist = HostCmd.split(";")
    #tt.multiThread(targetCmd,hostname,logDir)
    #sleep(1)
    ...
    #Test
    tt.startTest(testSuiteDict,testScenarioDict,commandDict,logDir,hostname,time=time_Mins)
